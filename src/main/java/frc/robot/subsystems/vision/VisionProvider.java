...
public abstract class VisionProvider {
  ...
  public List<Pair<PoseEstimate, Vector<N3>>> updateVisionMeasurements() {
    ...
    return visionMeasurements.stream()
        .sorted(
            (a, b) ->
                Double.compare(a.getFirst().timestampSeconds(), b.getFirst().timestampSeconds()))
        .toList();
  }
  ...
  /*
  Since records are immutable, we dont' modify it (albeit we could if it wasn't a primitive) thus we
  copy it
   */
  private PoseEstimate adjustPoseEstimate(String signalPath, PoseEstimate poseEstimate) {
    ...
    var timestampSeconds = Utils.getCurrentTimeSeconds() - (ntLatency + poseEstimate.latency());

    var newEstimate = new PoseEstimate(poseEstimate, timestampSeconds);

    writePoseEstimate(signalPath, newEstimate);
    return newEstimate;
  }
